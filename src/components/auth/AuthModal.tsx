...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  useEffect(() => {
    ...
    console.log('AuthModal useEffect: Running. isAuthenticated:', isAuthenticated, 'user:', user, 'isOpen:', isOpen, 'currentView:', currentView); // ADDED LOG
    if (
      isAuthenticated &&
      user &&
      isOpen &&
      ...
      console.log('AuthModal useEffect: User is authenticated and modal is open. Checking profile prompt status.'); // ADDED LOG
      console.log('AuthModal useEffect: user.hasSeenProfilePrompt:', user.hasSeenProfilePrompt); // ADDED LOG
      if (user.hasSeenProfilePrompt === false) {
        console.log('AuthModal useEffect: User needs to fill profile. Calling onProfileFillRequest.'); // ADDED LOG
        timer = setTimeout(() => {
          onProfileFillRequest('profile');
          onClose();
        }, 300);
      } else {
        console.log('AuthModal useEffect: User profile is complete or prompt seen. Closing AuthModal.'); // ADDED LOG
        timer = setTimeout(() => {
          onClose();
        }, 300);
      }
    } else {
      console.log('AuthModal useEffect: Conditions not met for profile prompt check. isAuthenticated:', isAuthenticated, 'user:', !!user, 'isOpen:', isOpen, 'currentView:', currentView); // ADDED LOG
    }
    return () => {
