...
  onShowAuth: () => void;
  onShowSubscriptionPlans: () => void; // Add new prop
}
...
        if (!subscription || (subscription.scoreChecksTotal - subscription.scoreChecksUsed) <= 0) {
...
      if (user) {
        await paymentService.useScoreCheck(user.id); // Decrement score check count
      }
    } catch (error) {
